...
class WalletManager {
  ...
  public:
	...
	// TODO: Properly document/place the functions below to their right place inside the .cpp/.h files
	
	// Create new random generated mnemonic phrase
	bip3x::Bip39Mnemonic::MnemonicResult createNewMnemonic();
	
	// Create an new root key based on the previous generated mnemonic phrase
	bip3x::HDKey createBip32RootKey(bip3x::Bip39Mnemonic::MnemonicResult phrase);
	
	// Create an public/private key pair based on the root key and rootPath
	// Root key and paths can be used to generate "infinite" accounts. see example below
	
	bip3x::HDKey createBip32Key(bip3x::HDKey rootkey, std::string rootPath);
	
	/* TODO: find an way to store the RootKey so the user don't need to re-enter his mnemonic phrase to create an second account based on the same mnemonic phrase
	 * Since we are storing the keys in keystore based files, we can only store one public/private key pair per time
	 * We provide the user with an list of addresses based on his rootkey.
	 * "m/44'/60'/0'/0" is the default root path for Ethereum
	 * "m/44'/60'/0'/0/0" will generated address 0xaaaaaaaaaaaaaaaaaa...
	 * "m/44'/60'/0'/0/1" will generated address 0xbbbbbbbbbbbbbbbbbb...
	 * "m/44'/60'/0'/0/2" will genereate address 0xcccccccccccccccccc...
	 * The user should be able to choose which path he wants to use
	 */
	std::vector<std::string> addressListBasedOnRootIndex(bip3x::HDKey rootkey, int64_t index);
	
	// Create an new keystore account based on the keypair generated by createBip32Key
