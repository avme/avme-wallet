  ...
  public:
    ...
    Q_INVOKABLE void createAccount(QString seed, int index, QString name, QString pass) {
      QtConcurrent::run([=](){
        ...
        Account a = QmlSystem.w.createAccount(seedStr, index, nameStr, passStr);
        if (!a.id.empty()) {
    ...
    Q_INVOKABLE void importLedgerAccount(QString address, QString path) {
      QmlSystem.w.importLedgerAccount(address.toStdString(), path.toStdString());
    }

    // Erase an Account
    Q_INVOKABLE bool eraseAccount(QString account) {
      return QmlSystem.w.eraseAccount(account.toStdString());
    }

    // Check if Account exists
    Q_INVOKABLE bool accountExists(QString account) {
      return QmlSystem.w.accountExists(account.toStdString());
    }
